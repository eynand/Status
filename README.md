...
- ✓ ניטור 14 שירותים פופולריים: ספוטיפיי, אפל מיוזיק, פייסבוק, אינסטגרם, וואטסאפ, יוטיוב, נטפליקס, דיסני+, Max (HBO), הוט, סלקום TV, yes, פרטנר TV, סטינג TV
- ✓ קטגוריות: מוזיקה, רשתות חברתיות, מסרים, וידאו, סטרימינג, טלוויזיה ישראלית
- ✓ **3 רמות אמינות שקופות**: גבוהה (API רשמי) / בינונית (CDN ו-API) / נמוכה (אתר תאגידי)
- ✓ שאיבה מ-APIs רשמיים של דפי הסטטוס + בדיקות multi-endpoint לשירותי streaming
- ✓ מדידת זמני תגובה לזיהוי האטות
...
המערכת שקופה לגבי מה היא בודקת. כל שירות מסומן באחת מ-3 רמות:

| רמה | מה נבדק | דוגמאות |
|-----|---------|---------|
| 🟢 **גבוהה** | API רשמי של דף הסטטוס - מדיווחי השירות עצמו | ספוטיפיי, אפל מיוזיק, יוטיוב |
| 🔵 **בינונית** | endpoints מרובים של ה-CDN/API של השירות בפועל. משקף תשתית streaming | נטפליקס, דיסני+, Max, פייסבוק, אינסטגרם, וואטסאפ |
| 🟡 **נמוכה** | זמינות אתר תאגידי בלבד. **לא בהכרח** משקף את זמינות ה-streaming/TV | הוט, סלקום TV, yes, פרטנר, סטינג |

**למה זה חשוב?** אתר של חברת תקשורת ישראלית רץ על תשתית **לגמרי נפרדת** מהתשתית של ה-streaming/TV. ה-WordPress של hot.net.il יכול לעבוד בזמן שה-VOD שבור, ולהפך. הסימון של "אמינות נמוכה" מבהיר זאת למשתמש - הסטטוס משקף רק זמינות בסיסית של החברה ברשת, לא של השירות עצמו.
...
| `ping` | בדיקת זמינות HTTP בסיסית (HEAD/GET) | Netflix, Disney+, Max, הוט, סלקום TV, yes |

**הערה על שירותי `ping`**: עבור שירותים שלא חושפים API ציבורי לסטטוס (כמו נטפליקס, דיסני+, וכל הפלטפורמות הישראליות), המערכת מבצעת בדיקת זמינות בסיסית - שולחת בקשת HTTP לאתר וכוללת:
- ✅ קוד 2xx/3xx = תקין
- ✅ קוד 4xx (כמו 403) = השרת חי, רק חוסם בקשות אוטומטיות
- ⚠️ Timeout = השירות איטי
- ❌ אין תגובה / 5xx = השירות לא נגיש
